...
├── eval_gaze_refinement.py
├── eval_vlm_multisegment.py
├── eval_gaze_refinement_multisegment.py
├── analysis/                  # 선택 분석/요약 유틸리티
│   ├── summarize_multiseg_results.py
│   └── compare_repframe_methods_multisegment.py
└── requirements.txt
```
...
The evaluation code is organized into two parallel tracks:

- **Single-segment track**
  - `eval_vlm_baseline.py` (Step 3-1): Gemini predicts one keyframe interval
  - `eval_gaze_refinement.py` (Step 3-2): post-process that interval with gaze velocity
- **Multi-segment track**
  - `eval_vlm_multisegment.py` (Step 3-3): Gemini predicts `1..N` keyframe intervals
  - `eval_gaze_refinement_multisegment.py` (Step 3-4): compare representative-frame rules inside each predicted interval

If your task has a single GT interval, use the single-segment track.
If your task has multiple GT intervals (for example, pick-place or double-object tasks),
use the multi-segment track.

The directory `analysis/` contains **optional analysis utilities**.
These scripts do not call Gemini and are not required for the main evaluation workflow.

- `analysis/summarize_multiseg_results.py`
  - Purpose: compact table summary across many `result_multiseg_*.json` files
  - Use it when you already finished several runs and want a quick comparison by
    `fps`, `caption`, `gaze`, and ordered-hit metrics
- `analysis/compare_repframe_methods_multisegment.py`
  - Purpose: compare multiple representative-frame heuristics inside predicted intervals
  - Use it only when midpoint vs min-velocity is not enough and you want to test
    alternatives such as plateau-center or hybrid methods
...
### 3-3. Multi-Segment VLM Evaluation (`eval_vlm_multisegment.py`)

Use this track when an episode has **multiple GT interaction intervals**.
Instead of returning one interval, Gemini is prompted to return `1..N`
non-overlapping intervals sorted by time.

#### Quick Start

```bash
# Evaluate all episodes in dataset/ with up to 2 predicted intervals
python eval_vlm_multisegment.py --max-intervals 2

# Evaluate only specific episodes
...
# Caption + gaze overlays, parallel workers
python eval_vlm_multisegment.py \
    --caption --gaze-annot \
    --video-fps 4 \
    --max-intervals 2 \
...
| `--episodes`          | all              | Comma-separated episode names to evaluate. |
| `--video-fps`         | `2`              | FPS hint sent to Gemini for video sampling. |
| `--caption`           | `false`          | Burn transcript captions into the input video. |
| `--gaze-annot`        | `false`          | Burn gaze dot overlay into the input video. |
| `--target-resolution` | `256`            | Resize before overlays. |
| `--max-intervals`     | `2`              | Maximum number of predicted intervals to return. |
| `--workers`           | `4`              | Number of episodes processed in parallel. |
| `--resume`            | `false`          | Skip episodes already present in JSONL results. |
| `--debug`             | `false`          | Save the exact video sent to Gemini. |

#### Output

- `eval_results_multiseg/{exp_tag}_results.jsonl`
- `eval_results_multiseg/{exp_tag}_summary.json`
- `results/result_multiseg_{model}_gaze_{true|false}_cap_{true|false}_fps{N}_max{M}.json`

The structured `result_multiseg_*.json` file is the main output and the input
to Step 3-4 below.
...
| Metric | Description |
| ------ | ----------- |
| `exact_count_match` | Predicted interval count equals GT interval count. |
| `segment_precision` | Fraction of predicted intervals matched to GT intervals. |
| `segment_recall` | Fraction of GT intervals matched by predicted intervals. |
| `segment_f1` | F1 over predicted-vs-GT interval matching. |
| `frame_iou` | IoU between the union of predicted frames and GT frames. |
| `pred_midpoint_hit_rate` | Midpoint-frame hit rate computed from predicted intervals. |

---

### 3-4. Multi-Segment Gaze Refinement (`eval_gaze_refinement_multisegment.py`)

This is the multi-segment counterpart of Step 3-2.
It does **not** call Gemini again. Instead, it reads the result file from Step 3-3
and compares representative-frame choices inside each predicted interval.

The two built-in methods are:

- `midpoint`: the center frame of the predicted interval
- `gaze_velocity`: the frame with minimum gaze angular velocity inside the interval

#### Quick Start

```bash
# Refine one multi-segment result file
python eval_gaze_refinement_multisegment.py \
    --vlm-results results/result_multiseg_gemini-3.1-flash-lite-preview_gaze_false_cap_false_fps4_max2.json

# Custom smoothing parameters
python eval_gaze_refinement_multisegment.py \
    --vlm-results results/result_multiseg_gemini-3.1-flash-lite-preview_gaze_true_cap_true_fps4_max2.json \
    --savgol-window 15 --savgol-poly 3
```
...
| Argument            | Default      | Description |
| ------------------- | ------------ | ----------- |
| `--vlm-results`     | *(required)* | Path to a `result_multiseg_*.json` file from Step 3-3. |
| `--dataset-dir`     | `./dataset`  | Path to the `dataset/` directory. |
| `--frame-tolerance` | `0,1,3,5`    | Frame tolerance values for hit-rate evaluation. |
| `--savgol-window`   | `11`         | Savitzky-Golay smoothing window for pitch/yaw. |
| `--savgol-poly`     | `2`          | Savitzky-Golay polynomial order. |
| `--verbose`         | `false`      | Print detailed per-episode information. |

#### Output

- `results/refined_result_multiseg_{...}.json`

This file stores interval-level and episode-level comparisons for both methods.

#### Main Metrics

| Metric | Description |
| ------ | ----------- |
| `exact_accuracy` | Interval-level exact hit rate. |
| `hit@k_rate` | Interval-level hit rate within `±k` frames of GT. |
| `any_hit_episode_rate` | Whether at least one predicted representative frame hits GT in an episode. |
| `all_interval_episode_rate` | Whether all predicted intervals hit GT in an episode. |
| `ordered_gt_hit_recall` | Ordered GT recall: `pred[i]` must hit `gt[i]`. |
| `ordered_all_hit_episode_rate` | Strict episode success: all predicted intervals must hit the corresponding GT interval in order. |

For tasks where interval order matters, `ordered_all_hit_episode_rate` is the
strictest and most informative metric.
